...
\ppv{l}{[0]}{A variable \texttt{l} stored in memory as \texttt{[0]}}{parameter_example}
...
\ppv{l}{[1]}{The variable \texttt{l} stored in memory as \texttt{[0]} after calling function \cref{parameter_passing:reassign}.}{parameter_reference_reassign}
...
\ppv{l}{[0, 1]}{The variable \texttt{l} stored in memory as \texttt{[0, 1]} after calling function \cref{parameter_passing:append}.}{parameter_reference_append}
...
\ppv{l}{[0]}{The variable \texttt{l} stored in memory as \texttt{[0]} after calling function \cref{parameter_passing:reassign} and \cref{parameter_passing:append}.}{parameter_value_l}
\ppv{l$'$}{[1]}{The variable \texttt{l$'$} stored in memory as \texttt{[1]} when calling function \cref{parameter_passing:reassign} and \cref{parameter_passing:append}.}{parameter_value_lprime}
...
\ppv{l}{[0]}{The variable \texttt{l} stored in memory as \texttt{[0]} after calling function \cref{parameter_passing:reassign}.}{parameter_object_reassign_l}
\ppv{l$'$}{[1]}{The variable \texttt{l$'$} stored in memory as \texttt{[1]} when calling function \cref{parameter_passing:reassign}.}{parameter_object_reassign_lprime}

When calling the \texttt{append} function the object is referenced and both \texttt{l} and \texttt{l$'$} are changed to \texttt{[0, 1]}.
Calling \texttt{append} manipulates the variables this is visualised in \cref{parameter_object_append}.

\begin{figure}[h]
   \centering
\begin{tikzpicture}
  \node[circle] (var1) at (0,5) {l};
  \node[circle] (var2) at (0,3) {l$'$};
  \node[draw, line width=0.05mm, minimum width=1cm,minimum height=1cm, outer sep=10pt] (mem1) at (1.5,5) {mem};
  \node[draw, line width=0.05mm, minimum width=1cm,minimum height=1cm, outer sep=10pt] (mem2) at (1.5,3) {mem};  
  \node (result) at (3,4) {[0, 1]};
  \draw[->] (var1) -- (mem1);
  \draw[->] (var2) -- (mem2);
  \draw[->] (result) -- (mem1);
  \draw[->] (result) -- (mem2);
